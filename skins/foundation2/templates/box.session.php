 ...
 *}
<div class="right text-right" id="box-session">
   {if $IS_USER}
   <a href="#" data-dropdown="session"><span class="show-for-medium-up">{$CUSTOMER.first_name} {$CUSTOMER.last_name} </span><span class="show-for-small-only">O</span></a><br>
   ...
   {else}
   <a href="{$STORE_URL}/login.html" class="nopad-sides">{$LANG.account.login}</a> / <a href="{$STORE_URL}/register.html">{$LANG.account.register}</a>
   {/if}
</div>
